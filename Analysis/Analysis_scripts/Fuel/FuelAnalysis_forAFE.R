...
#renaming macroplot column plot
fuels=rename(fuels, plot=`Macroplot`)
#creating new column called year
fuels <- fuels %>% mutate(year=NA)
#adding thousand hour rotten and sound fuel counts into new column
fuels$'1000hr'=fuels$X.3_Snd+fuels$X.3_Rot
#removing columns that start with X
fuels <- fuels %>% select(!starts_with("X"))

#LUMP 1990-92 together and make a note because they weren't full samples - unique to Saguaro!
#filling in year column based on monstatusord - must change according to years sampled for program
...
#fuels <- fuels %>% filter(year!='1992' & year!='1997') #TRY NOT DOING THIS
...
ggplot() +
  ...
  ggtitle("Fuel loading (tons/acre)")+facet_wrap(~Size_class, scales="free")
